...
const entryLeaf = (entry) => Buffer.from(`${entry.kind}\u0000${entry.path}\u0000${entry.sha256}`, 'utf8');
/** Merkle root over the entries. A single swapped report changes it. */
export function evidenceRoot(entries) {
...
export function extractStatus(text) {
    ...
    if (suite) {
        ...
        const status = failures + errors > 0 ? 'failed' : tests && tests > 0 ? 'passed' : 'unknown';
    ...
    try {
        ...
        if (failed && Number(failed[2]) > 0)
            return { status: 'failed', summary: `${failed[2]} failed` };
        if (passed)
            return { status: 'passed', summary: `${passed[2]} passed` };
        if (/\bFAIL(ED)?\b/.test(text) && !/\bPASS(ED)?\b/.test(text))
            return { status: 'failed' };
        if (/\bPASS(ED)?\b/.test(text))
    ...
    const summaryBits = [];
    for (const key of ['stages', 'checks', 'cases', 'tests']) {
        const value = obj[key];
        if (Array.isArray(value))
            summaryBits.push(`${value.length} ${key}`);
    }
    for (const key of ['passed', 'failed', 'total', 'version', 'artifactDigest']) {
        const value = obj[key];
        if (typeof value === 'string' || typeof value === 'number')
            summaryBits.push(`${key}=${value}`);
    }
    let status = 'unknown';
    if (typeof candidate === 'string') {
        const lower = candidate.toLowerCase();
        if (['pass', 'passed', 'ok', 'success', 'succeeded'].includes(lower))
            status = 'passed';
        else if (['fail', 'failed', 'failure'].includes(lower))
            status = 'failed';
        else if (['error', 'errored', 'infra-error'].includes(lower))
            status = 'error';
    }
    return { status, summary: summaryBits.slice(0, 4).join(', ') || undefined };
}
/** Walk a directory, skipping the usual noise. */
function walk(root, maxFiles = 400, depth = 0) {
    const out = [];
    if (depth > 6)
        return out;
    for (const item of readdirSync(root, { withFileTypes: true })) {
        if (out.length >= maxFiles)
            break;
        if (['node_modules', '.git', 'lib', 'dist', 'coverage'].includes(item.name))
            continue;
        const full = join(root, item.name);
        if (item.isDirectory())
            out.push(...walk(full, maxFiles - out.length, depth + 1));
        else
            out.push(full);
    }
    return out;
...
export function collect(options) {
    ...
    for (const file of files) {
        ...
        const kind = known ? known[1] : 'security-audit';
        let extracted = {};
...
export function readSubject(root, tarballPath) {
    const pkgPath = join(resolve(root), 'package.json');
    if (!existsSync(pkgPath))
        throw new Error(`no package.json under ${root}`);
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf8'));
    const subject = { name: pkg.name ?? '(unnamed)', version: pkg.version ?? '0.0.0' };
    try {
...
export function signPack(pack, privateKeyPem) {
 ...
 */
export function verifyPack(pack, root, now = Date.now()) {
    const issues = [];
    if (pack.schema !== EVIDENCE_SCHEMA)
        issues.push(`unexpected schema: ${String(pack.schema)}`);
    const rootValid = evidenceRoot(pack.entries) === pack.evidenceRoot;
    if (!rootValid)
        issues.push('evidenceRoot does not match the entries (a report was added, removed or reordered)');
    const digests = [];
    ...
    if (pack.signature && pack.signerPublicKey) {
        try {
            signatureValid = verifySig(null, Buffer.from(signable(pack), 'utf8'), createPublicKey(pack.signerPublicKey), Buffer.from(pack.signature, 'base64'));
        }
        catch (err) {
            issues.push(`signature could not be checked: ${err.message}`);
        }
        if (!signatureValid)
            issues.push('signature does not verify');
    }
    else {
        issues.push('pack is unsigned');
    }
    const present = new Set(pack.entries.map((e) => e.kind));
    const missingKinds = Object.keys(EVIDENCE_KINDS).filter((kind) => EVIDENCE_KINDS[kind].required && !present.has(kind));
    const failedKinds = [...new Set(pack.entries.filter((e) => e.status === 'failed' || e.status === 'error').map((e) => e.kind))];
    for (const kind of missingKinds)
        issues.push(`no ${kind} evidence in the pack (required)`);
    for (const kind of failedKinds)
        issues.push(`${kind} evidence reports a failure`);
    const stale = now > pack.expiresAt;
    if (stale)
        issues.push(`pack expired at ${new Date(pack.expiresAt).toISOString()}`);
...
export function renderEvidenceMarkdown(pack, verification) {
    const lines = [];
    ...
    if (pack.subject.tarballBytes)
        lines.push(`| tarball size | ${pack.subject.tarballBytes} bytes |`);
    ...
    if (verification) {
        ...
        lines.push('');
        if (verification.missingKinds.length > 0)
            lines.push(`- missing required evidence: ${verification.missingKinds.join(', ')}`);
        if (verification.failedKinds.length > 0)
            lines.push(`- failing evidence: ${verification.failedKinds.join(', ')}`);
    ...
    lines.push(`|---|---|---|---|---|`);
    for (const entry of pack.entries) {
        lines.push(`| ${entry.kind} | \`${entry.path}\` | ${entry.status ?? 'n/a'} | \`${entry.sha256.slice(0, 12)}…\` | ${entry.summary ?? ''} |`);
    }
    const absent = Object.keys(EVIDENCE_KINDS).filter((kind) => !pack.entries.some((e) => e.kind === kind));
    if (absent.length > 0) {
        lines.push('');
...
export function writePack(targetDirectory, pack, verification) {
