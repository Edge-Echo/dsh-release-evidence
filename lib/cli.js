...
import { EVIDENCE_KINDS, EVIDENCE_SCHEMA, collect, evidenceRoot, readPack, readSubject, renderEvidenceMarkdown, signPack, verifyPack, writePack, } from './evidence.js';
...
function collectOptions(args) {
    ...
    if (!Number.isFinite(maxAgeDays) || maxAgeDays <= 0)
        die('--max-age-days must be a positive number');
    ...
    switch (args.command) {
        case 'collect': {
            ...
            for (const entry of entries) {
                process.stdout.write(`  ${entry.kind.padEnd(17)} ${entry.path}  ${entry.status ?? ''} ${entry.summary ? `(${entry.summary})` : ''}\n`);
            }
            for (const note of skipped)
                process.stdout.write(`  skipped ${note}\n`);
            const missing = Object.keys(EVIDENCE_KINDS).filter((kind) => EVIDENCE_KINDS[kind].required && !entries.some((e) => e.kind === kind));
            if (missing.length > 0)
                process.stdout.write(`  missing required: ${missing.join(', ')}\n`);
            return;
        }
        case 'pack': {
            const { root, maxAgeMs } = collectOptions(args);
            ...
            if (!Number.isFinite(expiresDays) || expiresDays <= 0)
                die('--expires-days must be a positive number');
            let tarballPath;
            if (args.flags.has('pack-tarball')) {
                tarballPath = packTarball(root);
                if (!tarballPath)
                    process.stderr.write('warning: npm pack failed, continuing without a tarball digest\n');
            }
            const { entries, scanned } = collect({ root, maxAgeMs });
            if (entries.length === 0)
                die(`no evidence found under ${root}; run the release gates first`, 3);
            ...
            const pack = {
                ...
                environment: {
                    ...
                    arch: process.arch,
                    dshVersions: [...new Set(entries.map((e) => /dsh@?([\d.]+)/.exec(e.summary ?? '')?.[1]).filter(Boolean))],
                },
            };
            ...
            if (keyPath) {
                ...
                if (!check.signatureValid)
                    die('the signature did not verify after signing; refusing to write the pack', 3);
            }
        ...
        case 'verify': {
            ...
            if (json) {
                ...
                process.stdout.write(`  complete=${verification.complete} signature=${verification.signatureValid} ` +
                    `digests=${verification.digestsValid} root=${verification.rootValid} stale=${verification.stale}\n`);
                if (pack.signature)
                    process.stdout.write(`  signer key: ${pack.signerPublicKey?.split('\n')[1]?.slice(0, 32)}…\n`);
